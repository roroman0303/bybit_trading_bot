...
def main():
    ...
    equity=cfg['start_capital']; state=load_kv(session,'live_state') or {}
    ...
    while True:
        ...
        if signal:
            ...
            order=broker.enter_market(signal, qty)
            entry_price=float(row['close']); taker_fee=cfg['taker_fee']; fee_in=entry_price*qty*taker_fee
            sl_dist=cfg['sl_atr_mult']*atr_val; tp_dist=cfg['tp_atr_mult']*atr_val
            ...
            pnl=(entry_price-exit_price)*qty if signal=='short' else (exit_price-entry_price)*qty
            ...
            logger.info(f'LIVE {signal} pnl={pnl:.2f} equity={equity:.2f}')
            save_kv(session, 'live_state', {'equity': equity})
        time.sleep(5)
